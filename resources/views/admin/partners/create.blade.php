...
@section('page_subtitle', 'Daftarkan mitra pendukung AmikomEventHub.')
    ...
    <form action="{{ route('admin.partners.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" 
                placeholder="Contoh: Universitas Amikom, Tokopedia"
            ...
            <div class="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center hover:border-indigo-400 transition"
                ...
                <label for="logo-input" class="cursor-pointer">
                    <div id="upload-placeholder">
                        ...
                        <p class="font-bold text-slate-700 mb-1">Klik untuk Upload Logo</p>
                        <p class="text-xs text-slate-500">PNG, JPG, JPEG, atau SVG (Maks. 2MB)</p>
                    </div>
        ...
        <div>
            ...
            @error('website_url') 
                <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span> 
            @enderror
            <p class="text-xs text-slate-400 mt-2">URL website resmi partner (akan digunakan sebagai link logo di homepage).</p>
        </div>
...
<script>
    function previewLogo(event) {
        ...
        if (file) {
            // Validasi ukuran (2MB = 2 * 1024 * 1024)

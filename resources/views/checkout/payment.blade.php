    ...
    document.getElementById('pay-button').onclick = function () {
        snap.pay('{{ $transaction->snap_token }}', {
            ...
            onPending: function(result){
                // Jika pending (misal belum transfer VA), redirect ke halaman sukses (tapi status pending)
                window.location.href = "{{ route('checkout.success', $transaction->order_id) }}";
            },
            onError: function(result){
                // Jika error, tampilkan alert dan jangan redirect
                alert("Pembayaran Gagal! Silakan coba lagi.");
            },
            onClose: function(){
                // Tampilkan notifikasi yang lebih informatif
                const userChoice = confirm(
                    "Anda menutup jendela pembayaran.\n\n" +
                    "Pesanan Anda masih dalam status PENDING.\n\n" +
                    "Pilih OK untuk kembali ke halaman pembayaran dan mencoba lagi,\n" +
                    "atau pilih Cancel untuk melihat daftar tiket Anda."
                );
                
                if (userChoice) {
                    // User memilih untuk tetap di halaman payment (refresh)
                    window.location.reload();
                } else {
                    // User memilih untuk ke halaman tiket saya

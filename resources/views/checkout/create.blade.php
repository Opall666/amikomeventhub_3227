...
<main class="max-w-3xl mx-auto px-6 py-20">
    ...
    <!-- Flash Message Error -->
    @if(session('error'))
        <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-xl mb-6 font-bold text-sm">
            {{ session('error') }}
        </div>
    @endif

    <form action="{{ route('checkout.store', $event->id) }}" method="POST" class="grid grid-cols-1 gap-8">
        @csrf
        <!-- Hidden Input untuk Event ID -->
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="flex gap-6 items-start">
                @if($event->poster_path)
                    <img src="{{ asset('storage/' . $event->poster_path) }}" alt="Event" class="w-24 h-24 rounded-2xl object-cover">
                @else
